...
import org.manlier.recommend.entities.{Component, History, HistoryWithName, User}
...
object TestDataGenerator {
  ...
  def generateComponentTuples(num: Int, components: IndexedSeq[Component]): immutable.IndexedSeq[(Component, Component)] = {
    for (_ <- 0 until num)
      yield (components(math.floor(math.random * components.length).toInt)
        , components(math.floor(math.random * components.length).toInt))
  }
